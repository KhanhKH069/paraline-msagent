...
from PyQt6.QtWidgets import (
    ...
    QSizePolicy, QTextEdit, QPushButton, QComboBox,
)

from client.ui.components.helpers import card, label

logger = logging.getLogger("meeting_ai.chat")
...
class WhisperSTTWorker(QThread):
    ...
    """
    transcribed = pyqtSignal(str, str)   # (kết quả STT, detected_language)
    ...
    def run(self):
        try:
            ...
            if text:
                self.transcribed.emit(text, lang)
            else:
...
class FrameChat(QWidget):
    ...
    # Tín hiệu đẩy lên main_window: (text, src_lang, tgt_lang)
    send_requested    = pyqtSignal(str, str, str)
    ...
    def _build(self):
        ...
        self._combo_src.addItem("🌐 Tự động",     "auto")
        ...
        self._combo_src.addItem("🇨🇳 Chinese",    "zho_Hans")
        self._combo_src.addItem("🇰🇷 Korean",     "kor_Hang")
        self._combo_src.addItem("🇫🇷 French",     "fra_Latn")
        self._combo_src.addItem("🇩🇪 German",     "deu_Latn")
        self._combo_src.setFixedHeight(26)
        top_row.addWidget(self._combo_src)

        arrow_lbl = label("→", "chat_input_label")
        top_row.addWidget(arrow_lbl)

        tgt_lbl = label("ĐÍCH:", "chat_input_label")
        top_row.addWidget(tgt_lbl)

        self._combo_tgt = QComboBox()
        self._combo_tgt.setObjectName("combo_tgt_lang")
        self._combo_tgt.addItem("🇺🇸 English",    "eng_Latn")
        self._combo_tgt.addItem("🇯🇵 Japanese",   "jpn_Jpan")
        self._combo_tgt.addItem("🇻🇳 Tiếng Việt", "vie_Latn")
        self._combo_tgt.addItem("🇨🇳 Chinese",    "zho_Hans")
        self._combo_tgt.addItem("🇰🇷 Korean",     "kor_Hang")
        self._combo_tgt.addItem("🇫🇷 French",     "fra_Latn")
        self._combo_tgt.addItem("🇩🇪 German",     "deu_Latn")
        self._combo_tgt.setFixedHeight(26)
        top_row.addWidget(self._combo_tgt)
        top_row.addStretch()
        ...
        self._text_input.setPlaceholderText("Nhập tin nhắn (Enter để gửi, Shift+Enter xuống dòng)…")
        self._text_input.setFixedHeight(68)
        self._text_input.setAcceptRichText(False)
        self._text_input.send_requested.connect(self._on_send_clicked)
    ...
    def set_target_language(self, lang_code: str):
        """Đồng bộ ngôn ngữ đích của Chat từ control bar ngoài."""
        if not lang_code or lang_code == "auto":
            return
        idx = self._combo_tgt.findData(lang_code)
        if idx >= 0:
            self._combo_tgt.setCurrentIndex(idx)
    ...
    def _on_send_clicked(self):
        ...
        if not text:
            return
        combo_val = self._combo_src.currentData() or "vie_Latn"
        if combo_val == "auto":
            src_lang = getattr(self, "_detected_src_lang", "vie_Latn")
        else:
            src_lang = combo_val
        tgt_lang = self._combo_tgt.currentData() or "eng_Latn"
        self._text_input.clear()
        self.send_requested.emit(text, src_lang, tgt_lang)
    ...
    def _on_stt_done(self, text: str, detected_lang: str):
        _WHISPER_TO_NLLB = {
            "vi": "vie_Latn",
            "ja": "jpn_Jpan",
            "en": "eng_Latn",
        ...
        if self._combo_src.currentData() == "auto":
            self._detected_src_lang = _WHISPER_TO_NLLB.get(detected_lang, "vie_Latn")
            self._set_status(f"✅ Nhận dạng ({detected_lang.upper()})", visible=True)
        else:
