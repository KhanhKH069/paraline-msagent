...
class DeviceLoaderThread(QThread):
    ...
    def run(self):
        ...
        self.devices_loaded.emit(results)
...
class SplashScreen(QWidget):
    ...
    def _populate_devices(self):
        ...
        self._combo_device.addItem("Đang quét thiết bị...", userData=("", ""))
        self._combo_device.setEnabled(False)
        if hasattr(self, '_btn_refresh'):
            self._btn_refresh.setEnabled(False)
            
        self._loader_thread = DeviceLoaderThread()
        self._loader_thread.devices_loaded.connect(self._on_devices_loaded)
        self._loader_thread.start()

    def _on_devices_loaded(self, results):
        self._combo_device.clear()
        if results:
            for text, data in results:
                self._combo_device.addItem(text, userData=data)
        else:
