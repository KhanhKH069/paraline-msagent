...
logger = logging.getLogger("meeting_ai.audio")
...
class AudioManager:
    ...
    def start(
        self,
        inbound_cb: Callable[[str, bool], None],
        inbound_device: Optional[str] = None,
        outbound_cb: Optional[Callable] = None,
        level_cb: Optional[Callable[[float], None]] = None,
        **kwargs,
    ):
        """
        ...
        outbound_cb: callback âm thanh từ micro (tùy chọn).
        level_cb: callback biên độ âm thanh thời gian thực (RMS float).
        """
        self._running = True

        out_device_idx = sd.default.device[1]
        logger.info("[AUDIO] Khởi động AudioManager...")
        logger.info("[AUDIO] Playback device: Loa mặc định [idx: %s]", out_device_idx)

        # ── Khởi động InboundAudioManager ────────────────────
        self._inbound.start(
            callback=inbound_cb,
            device_name=inbound_device,
