...
# Trên CPU: dùng float32 vì float16 gây crash hoặc sinh text rác trên phần lớn CPU
# Trên GPU (CUDA): giữ float16 để tiết kiệm VRAM và tăng tốc
if DEVICE == "cuda":
    _dtype = torch.float16
else:
    _dtype = torch.float32
    logger.info("⚠️  CPU mode: dùng float32 (không có CUDA)")
