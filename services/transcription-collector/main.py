...
logger = logging.getLogger("meeting_ai.collector")

DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://meeting:meeting_strong_password@postgres:5432/meeting")

app = FastAPI(title="Meeting Transcription Collector")
...
async def store_segment(seg: SegmentIn):
    ...
    async with _pool.acquire() as conn:
        try:
            sess_uuid = uuid.UUID(seg.session_id)
            await conn.execute("""
                INSERT INTO meeting_sessions (session_id, status, inbound_src_lang, outbound_tgt_lang, created_at)
                VALUES ($1, 'active', $2, $3, NOW())
                ON CONFLICT (session_id) DO NOTHING
            """, sess_uuid, seg.src_lang, seg.tgt_lang)
        except Exception as e:
            logger.warning(f"Could not ensure session {seg.session_id}: {e}")
