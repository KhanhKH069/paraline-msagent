...
# (Optional) For Microphone/System Audio real-time inference
pip install sounddevice scipy

# (Optional) For auto-translation to English/Japanese
...
### Microphone & System Audio Inference (Real-time)

Transcribe speech directly from your microphone or capture system audio (e.g., from YouTube videos, meetings) in real-time. Requires the `sounddevice` package, and potentially `scipy` for high-quality audio resampling.

*(New ✨)* You can now auto-translate transcripts into English and Japanese by appending the `--translate` flag (requires `deep-translator`).

```bash
# 1. List all available input devices (Look for Mic or ⭐ LOOPBACK / Stereo Mix)
python infer_mic.py --list-devices

# 2. Use the default microphone
python infer_mic.py

# 3. Use Loopback/Stereo Mix by the exact index found from the list step
# (e.g. if index is 27) and auto-translate
python infer_mic.py --device 27 --translate
```

### Video Inference & Audio Extraction

Extract audio from video files (`.mp4`, `.mkv`, etc.) and transcribe directly. Requires `ffmpeg` installed on your system.

*(New ✨)* You can also translate video transcriptions on-the-fly using the `--translate` flag.

```bash
# Transcribe a single video file
python infer_video.py --video data/sample.mp4

# Transcribe multiple video files with INT8 quantization
python infer_video.py --video v1.mp4 v2.mp4 --quantize int8

# Real-time simulation (process in 5-second chunks)
python infer_video.py --video data/sample.mp4 --realtime --chunk-size 5

# Transcribe and translate to English/Japanese
python infer_video.py --video data/sample.mp4 --translate

# Only extract audio to WAV, without running recognition
